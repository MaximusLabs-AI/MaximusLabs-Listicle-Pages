...
function Reviews({agency}: {agency: RecordValue}) {
  ...
  return (
    <>
      ...
      {agency.reviewNote ? <p className="legend">{agency.reviewNote}</p> : null}
    </>
  )
}
    ...
    <div className={mobile ? 'mobile-preview' : ''}>
      ...
      <main className="wrap" id="directory">
        <header className="masthead">
          <div className="crumbs"><span>{page.publisherName || 'MaximusLabs.ai'} · Agency directory · {page.verticalLabel}</span><span>Last reviewed {longDate(page.reviewedAt)} · {entries.length} firms evaluated</span></div>
